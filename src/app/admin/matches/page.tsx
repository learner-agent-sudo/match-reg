    ...
    if (playoffFormat === "championship_random") {
      ...
      const matchRows = playoffMatches.map((m, i) => {
        ...
        if (m.stage === "semi_final" && i === 0 && shuffled.length >= 2) {
          row.home_team_id = shuffled[0].id;
          row.away_team_id = shuffled[1].id;
          row.placeholder_home = null;
          row.placeholder_away = null;
        }
        if (m.stage === "semi_final" && i === 1 && shuffled.length >= 4) {
          row.home_team_id = shuffled[2].id;
          row.away_team_id = shuffled[3].id;
          row.placeholder_home = null;
          row.placeholder_away = null;
        }
        return row;
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-4">
          <h2 className="text-lg font-semibold">Match Scheduling</h2>
      ...
      {showGroupSetup && (
        <div className="bg-white rounded-lg shadow-sm p-6 space-y-4">
          <h3 className="font-semibold">Group Setup</h3>
            ...
            <div key={group.id} className="border rounded-md p-3">
              ...
              <div className="flex flex-wrap gap-2 mb-2">
                {group.group_teams.map((gt) => (
                  <span key={gt.team_id} className="inline-flex items-center gap-1 px-2 py-1 bg-gray-100 rounded text-sm">
                    {getTeamName(gt.team_id)}
                    <button
                      onClick={() => removeTeamFromGroup(group.id, gt.team_id)}
          ...
          <div className="border-t pt-4">
            <h4 className="font-medium text-sm mb-2">Generate Playoff Bracket</h4>
      ...
      {groupMatches.length > 0 && (
        <div className="bg-white rounded-lg shadow-sm p-6">
          <h3 className="font-semibold mb-4">Group Stage Matches</h3>
      ...
      {playoffMatches.length > 0 && (
        <div className="bg-white rounded-lg shadow-sm p-6">
          <h3 className="font-semibold mb-4">Playoff Matches</h3>
          <div className="space-y-2">
      ...
      {matches.length === 0 && (
        <p className="text-sm text-gray-500 bg-white rounded-lg shadow-sm p-6">
    ...
    <div className="border rounded-md p-3 text-sm">
      <div className="flex justify-between items-center mb-2">
        <div className="flex items-center gap-3">
          <span className="text-xs px-2 py-0.5 rounded bg-gray-100 text-gray-600">{stageLabel}</span>
        ...
        <button onClick={() => onDelete(match.id)} className="text-xs text-red-500 hover:text-red-700">
          Delete
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
        <select
          value={match.time_slot_id ?? ""}
          onChange={(e) => onAssign(match.id, "time_slot_id", e.target.value)}
          className="text-xs border border-gray-300 rounded px-2 py-1"
        >
          <option value="">Assign time slot...</option>
          {timeSlots.map((s) => (
            <option key={s.id} value={s.id}>
              {s.date} {s.start_time.slice(0, 5)} ({s.pitch?.name})
            </option>
          ))}
        </select>
        ...
        <select
          ...
          onChange={(e) => onAssign(match.id, "referee_id", e.target.value)}
          className="text-xs border border-gray-300 rounded px-2 py-1"
        >
        ...
        <select
          value={match.runner_id ?? ""}
          onChange={(e) => onAssign(match.id, "runner_id", e.target.value)}
          className="text-xs border border-gray-300 rounded px-2 py-1"
        >
